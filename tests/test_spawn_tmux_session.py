...
Terminal — and UNLIKE raw conhost, where the SGR 'm' terminator leaks into stdin
(Enter inserts a literal 'm'). A named session is also durable + supervisable:
the sidecar/watchdog wake it via `tmux send-keys -t <session>`, and a popup
window is just a viewport (`tmux attach`) onto a session that survives every
window close.

The ACTUAL tmux + wt.exe calls need a real native-Windows box; these tests pin
the DECISION logic (create vs attach vs skip, interactive vs headless) by mocking
...
def _drive(monkeypatch, *, platform="win32", tmux=TMUX, wt=WT, in_tmux=None,
    ...
    """Drive `_launch_via_tmux` with tmux/wt/subprocess mocked.
    ...
    `Popen` (the attach viewport) is a bare mock so callers can assert on it.
    """
...
def test_non_windows_never_uses_tmux(monkeypatch):
    r, run, popen = _drive(monkeypatch, platform="linux")
    assert r is False
...
def test_inside_tmux_skips_is_the_loop_breaker(monkeypatch):
    ...
    r, run, popen = _drive(monkeypatch, in_tmux="/tmp/tmux-1000/default,9,0")
    assert r is False
    run.assert_not_called()
    popen.assert_not_called()
...
def test_spawn_marker_skips_belt_and_suspenders(monkeypatch):
    ...
    r, run, popen = _drive(monkeypatch, spawn_marker="1")
    assert r is False
    run.assert_not_called()
...
def test_no_tmux_on_path_falls_through(monkeypatch):
    ...
    r, run, popen = _drive(monkeypatch, tmux=None)
    assert r is False
    popen.assert_not_called()
...
def test_existing_session_attaches_in_place_no_create(monkeypatch):
    # Session already exists + interactive => attach IN PLACE (this console),
    # DON'T create a second cell. claude is already running in the session.
    r, run, popen = _drive(monkeypatch, session_exists=True)
    assert r is True
    cmds = _cmds(run)
    assert any("has-session" in c for c in cmds)
    assert not any("new-session" in c for c in cmds)  # no second cell
    # in-place attach via subprocess.run (NOT a popped window)
    assert [TMUX, "attach", "-t", SESSION] in cmds
    popen.assert_not_called()


def test_absent_session_creates_then_attaches_in_place(monkeypatch):
    r, run, popen = _drive(monkeypatch, session_exists=False)
    assert r is True
    cmds = _cmds(run)
    ...
    # then attach in place (same console, no popped window)
    assert [TMUX, "attach", "-t", SESSION] in cmds
    popen.assert_not_called()
...
def test_absent_session_headless_creates_detached_no_attach(monkeypatch):
    ...
    r, run, popen = _drive(monkeypatch, session_exists=False, isatty=False)
    assert r is True
    cmds = _cmds(run)
    assert any("new-session" in c for c in cmds)
    assert [TMUX, "attach", "-t", SESSION] not in cmds  # detached, no attach
    popen.assert_not_called()
...
def test_existing_session_headless_is_a_noop_handoff(monkeypatch):
    ...
    r, run, popen = _drive(monkeypatch, session_exists=True, isatty=False)
    assert r is True
    cmds = _cmds(run)
    assert not any("new-session" in c for c in cmds)
    assert [TMUX, "attach", "-t", SESSION] not in cmds
    popen.assert_not_called()
...
def test_create_failure_falls_through(monkeypatch):
    ...
    # never pop a half-attached window.
    boom = subprocess.CalledProcessError(1, "tmux")
    r, run, popen = _drive(monkeypatch, session_exists=False, new_session_exc=boom)
    assert r is False
    popen.assert_not_called()
