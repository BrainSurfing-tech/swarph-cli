...
- **The A2 destructive respawn path** (`swarph watchdog`, dormancy recovery) — unchanged, per the 2026-07-04 spec's own boundary. This design is about keeping the *monitor/heartbeat* process supervised, a layer below where A2 operates; conflating the two would re-couple something the watchdog spec deliberately decoupled.
...
- **A single implementation of C** — explicitly three backends; a design that ships only the Linux one repeats #439's own history (filed, never followed up, because "Linux-only" made it easy to deprioritize once the immediate fire was out).
...
1. Does `wake_route`/`wake_sinks` belong on the mesh-gateway's peer capabilities (cross-repo, needs coordination with whoever owns `mesh-gateway`) or should swarph-cli maintain its own local declaration that a separate sync step pushes to the gateway? The cross-cutting requirement section above assumes the former; flagging it as a real design fork, not a settled fact.
2. Escalation (`B`)'s `DEGRADED` state — does it live purely as peer capability metadata, or does it need its own event/alert delivery path? If the latter, it has the exact same "how do you wake someone to see it" problem this whole design exists to solve, one level up — worth naming explicitly rather than assuming a `DEGRADED` flag is self-evidently seen.
3. `C`'s three backends: build and land together, or ship Linux/WSL first (where the systemd primitive and its traps are already fully characterized) and gate macOS/Windows behind their own CI-verified passes? Given #439's own history (Linux-only made it easy to deprioritize), leaning toward "no partial ship, all three or the card stays open" — but that's a real cost/sequencing tradeoff, not a unilateral call.
