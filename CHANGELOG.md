...
## Unreleased
- hooks (#830): install capability check asks PATH's `swarph codegraph-hook --supported-events` out of process — the in-process import shipped in #408 compared two literals in the same package and could not see install/PATH skew. Loud unknown events + version stamp unchanged.
